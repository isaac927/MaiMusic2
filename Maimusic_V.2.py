...
class audio_app:
    ...
    def audio_loop(self):
        ...
        while self.is_playing and self.stream_pos < len(self.audio_data):
            if self.is_paused or self.is_dragging:  # pause audio output during drag
                time.sleep(0.05)
                continue
    ...
    def load_file(self):
        ...
        if file_path:
            ...
            self.stream_pos = 0
            self.audio_duration = len(self.sound)  # in ms
    ...
    def update_progress_bar(self):
        ...
        time_str = f"{int(current_ms // 1000)} / {int(self.audio_duration // 1000)} sec"
    ...
    def open_file_dialog(self):
        ...
        if file_path:
            ...
            self.audio_data = self.sound.raw_data
            self.stream_pos = 0
            self.audio_duration = len(self.sound)  # in milliseconds
            self.file_label.config(text=os.path.basename(file_path))
            self.play_btn.config(text="▶️")
            self.update_progress_bar()
    ...
    def spleeter_seperation(self, sound):
        pass
        # Placeholder for Spleeter separation logic
